#ifndef MODIFIED_NEWTON_METHOD_H_INCLUDED
#define MODIFIED_NEWTON_METHOD_H_INCLUDED
#include "functions.h"
#include <iomanip>
...
В отличие от обычного метода Ньютона, здесь при достижении условия выхода из цикла, мы фиксируем производную
при подсчете корня.*/
    void modified_newton_method (double a, double b, double epsilon)
    {
        ...
        else
        {
            ...
            double constDerivative = getDerivativeValue(previous_solut);
            double current_solut = previous_solut -(func(previous_solut)/constDerivative);

            while(fabs(current_solut - previous_solut) >= epsilon)
            {
                previous_solut = current_solut;
                current_solut = previous_solut - (func(previous_solut) / constDerivative);
                ++counter;
            }
...
#endif // MODIFIED_NEWTON_METHOD_H_INCLUDED
